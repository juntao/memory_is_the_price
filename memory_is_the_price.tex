...
OpenRouter's public endpoints API we assemble a panel of 119 provider--model
...
behind its largest models~\cite{r19}), but for the 35 providers in our
...
123 raw observations. One duplicate listing and one stale row were removed
(\S\ref{sec:verify}), and models left with fewer than three qualifying
providers were then dropped, which excluded one small panel. The final panel
comprises \textbf{119 observations across 18 models (12 MoE spanning
sparsity ratios $E/k$ from 4 to 128, and 6 dense) from 35 distinct
providers}.
...
Dense models take active = total and $E/k = 1$.

\subsection{Verification pass}
\label{sec:verify}

Two mechanical rules produce the final panel from the raw rows, and both
are checkable against the archived data. First, the unit of observation is
one quote per provider per model; where a provider lists multiple endpoints
for the same model, the cheapest is kept, which is the price a customer
comparing providers would face. Exactly one pair is affected, DeepInfra's
fp8 and bf16 endpoints for Llama-3.1-8B, and no other duplicate pair exists
in the panel. Second, delisted rows are dropped; exactly one was found, a
SiliconFlow Qwen2.5-7B price, and its removal left that panel below the
three-provider threshold and removed the model from the study. In addition,
the panel's single most extreme value, DeepInfra's \$0.10/M output price on
Qwen3-235B-A22B, roughly eight times below its panel median, was confirmed
on DeepInfra's own price page~\cite{r18}. Quantization does not explain the
widest within-model spreads: in the second- and third-widest panels
(Llama-3.1-8B and Llama-3.3-70B), the minimum and maximum prices are both
fp8 offerings, and the widest (DeepSeek V3) separates a neocloud's
discounted listing from a hyperscaler's managed enterprise offering.
...
($\log_{10} p = -1.585 + 0.735\,\log_{10}(\text{active})$). Every MoE model
sits above it, by 2.2$\times$ to 8.5$\times$, and the excess grows with the
...
token for all 18 models on log-log axes, with dense models as green circles
and MoE models as red triangles. The dashed line is the dense-only OLS fit,
which is the price schedule a compute-priced market would apply to all
models. Every MoE
model sits above the line, and the vertical gap for DeepSeek R1 (6.5$\times$)
is annotated.}
\label{fig:level}
\end{figure}

The regression quantifies the pattern (Table~\ref{tab:ols}) and rejects H1
decisively. Compute pricing predicts $c_2 = 0$; the estimate is 0.541 with
$t = 8.7$. The sparsity-ratio elasticity is large, precisely estimated, and
...
$\sim$410B-equivalent dense model rather than the 37B model its compute would
suggest.

\begin{table}[t]
\centering
\caption{The table reports OLS estimates of the hedonic regression of log
median output price on architecture ($n = 18$ models, $R^2 = 0.875$). H1
...
models cost 3--12$\times$ more than dense models. The per-model premium over the dense line tracks the sparsity ratio: Kimi K2 (31$\times$ sparse) sits 7.5$\times$ above the line, MiniMax M2 (23$\times$) sits 8.5$\times$ above, and GPT-OSS-120B (23$\times$) sits 5.8$\times$ above. The mildly sparse Llama-4 Scout and GPT-OSS-20B (6$\times$) sit $\sim$2.2$\times$ above.
...
\midrule
Kimi K2          & 2.50  & 32   & 1{,}000 & 0.078 & 7.5 & yes \\
DeepSeek R1      & 2.39  & 37   & 671     & 0.065 & 6.5 & yes \\
DeepSeek V3.1    & 1.30  & 37   & 671     & 0.035 & 3.5 & yes \\
MiniMax M2       & 1.20  & 10   & 230     & 0.120 & 8.5 & yes \\
DeepSeek V3      & 1.12  & 37   & 671     & 0.030 & 3.0 & yes \\
GLM 4.5 Air      & 0.98  & 12   & 106     & 0.082 & 6.1 & yes \\
Llama 4 Maverick & 0.87  & 17   & 400     & 0.051 & 4.2 & yes \\
Qwen3 235B-A22B  & 0.78  & 22   & 235     & 0.035 & 3.1 & yes \\
GPT-OSS 120B     & 0.50  & 5.1  & 117     & 0.098 & 5.8 & yes \\
Llama 4 Scout    & 0.47  & 17   & 109     & 0.027 & 2.2 & yes \\
Qwen3 30B-A3B    & 0.30  & 3.3  & 30.5    & 0.091 & 4.8 & yes \\
GPT-OSS 20B      & 0.15  & 3.6  & 21      & 0.042 & 2.2 & yes \\
...
Llama 3.1 8B     & 0.08  & 8.0  & 8.0     & 0.010 & -- & no  \\
\bottomrule
\end{tabular}
\end{table}
...
\emph{(b) The price floor is set by vertically integrated providers; hyperscalers list at
...
alone, the seventh by the model's owner. Figure~\ref{fig:floors} plots every
provider quote in these panels and marks each panel's floor; the floor
marker is a neocloud or the model's owner in every row, and every
hyperscaler listing sits at or above it. DeepInfra is the archetype of the class. It raised \$107M in Series~B funding and owns and operates its GPU infrastructure (NVIDIA Blackwell B300) across eight U.S.\ data centers. It publishes owned-hardware economics of $\sim$\$1.98/GPU-hour against \$6.50 for equivalent rented public-cloud capacity, alongside a collaboration with NVIDIA on Dynamo disaggregated serving~\cite{r12}. By contrast, Azure AI Foundry and Amazon
...
\caption{The figure plots every provider's output price for the seven
panels of models with 200B+ total parameters (log scale). The lowest quote
in each panel (filled triangle, labeled) is posted by a vertically
integrated neocloud in six of seven panels and by the model's owner in the
seventh. Hyperscaler listings (squares) sit at or above the floor in every
panel where they appear, and the remaining providers (open circles) fill
the range between.}
\label{fig:floors}
\end{figure}
...
(hatched bars) versus dense panels (solid bars).}
\label{fig:disp}
\end{figure}
...
of a 671B/37B model pays approximately as if for a $\sim$410B dense model.
...
(\path{pd_prices.json}, 123 raw rows; 119 qualifying observations),
