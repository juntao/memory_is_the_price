...
$R^2 = 0.87$ with a sparsity elasticity of 0.554 ($t = 8.3$), which is 83\% of the active-parameter elasticity. MoE models are therefore billed most of the way toward their total memory footprint, and customers keep almost none of the advertised compute discount. The structure of the market explains why. Realizing MoE's compute savings requires expert-parallel decoding pools far larger than a single node. As a result, the price floor of every large-MoE panel is set by a vertically integrated ``neocloud,'' and hyperscalers list identical models at 2--5$\times$ that floor. Provider participation collapses with model footprint, which compresses the observed price dispersion through selection.
...
regression~\eqref{eq:hedonic} by ordinary least squares ($n = 18$). To test H2 (\S\ref{sec:h2}) we proceed in four steps. We identify each panel's floor-setting provider and its class. We compare hyperscaler and GPU-cloud medians within panels listed on both. We relate provider participation to model footprint. Finally, we measure within-model dispersion (the coefficient of variation of output price) for MoE versus dense panels, including a robustness split that excludes hyperscaler and edge endpoints. All statistics are computed
...
models cost 3--12$\times$ more than dense models. The per-model premium over the dense line tracks the sparsity ratio: Kimi K2 (31$\times$ sparse) sits 7.7$\times$ above the line, MiniMax M2 (23$\times$) sits 9.3$\times$ above, and GPT-OSS-120B (23$\times$) sits 6.6$\times$ above, while the mildly sparse Llama-4 Scout and GPT-OSS-20B (6$\times$) sit $\sim$2.5$\times$ above.
