...
expert-parallel decoding pools~\cite{deepseek,lmsys}. H2 predicts that the
price floor of each large-MoE model is set by a massively parallel,
vertically integrated provider, a specialized ``neocloud'' or the model's
own vendor, and that general-purpose clouds price above the floor. A
corollary concerns dispersion: providers whose latent cost exceeds the
competitive price tend not to list, so the observed spread for large MoE
should be compressed through selection~\cite{heckman}. We do not use raw
participation counts as a test, because they are confounded by demand for
newer models.
...
three-provider threshold, 46 models qualify. The eight DeepSeek panels are
...
sets a floor, and hyperscaler listings sit 1.2--8.8$\times$ above it. MoE
price dispersion remains \emph{compressed} rather than widened (median
coefficient of variation 0.23 versus 0.35 for dense): providers whose latent
cost exceeds the competitive price tend not to list, so the observed prices
are a selected sample~\cite{heckman}. The
