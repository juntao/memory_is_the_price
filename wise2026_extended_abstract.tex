...
over 121 provider--model price observations rejects the claim: the market
bills MoE models most of the way toward their total memory footprint
(sparsity elasticity 0.554, $t = 8.3$). Market structure explains why:
capturing MoE's compute savings requires expert-parallel serving pools far
larger than one node, so price floors belong exclusively to vertically
integrated neoclouds, hyperscalers list identical models at 2--5$\times$ the
floor, and provider participation collapses with model footprint.
...
customers pay only for active compute. Switch Transformers promised
``outrageous numbers of parameters --- but a constant computational
cost''~\cite{fedus}, and Mistral stated that Mixtral ``processes input and
...
observable proxy. Open-weight serving approximates the textbook competitive
case (the weights are free, the software is open source, entry is
unrestricted, and dozens of providers quote side by side on the OpenRouter
marketplace), so competition disciplines prices toward marginal cost, and
sustained price structure identifies cost structure~\cite{hayek}.
...
provider price pages. After exclusions, the panel comprises 121 observations
across 18 models (12 MoE, 6 dense) from roughly 40 providers, joined to
...
$\sim$430B dense model, and every MoE model sits 2.4--9.3$\times$ above the
...
($n = 18$ models, $R^2 = 0.866$). H1 predicts $c_2 = 0$; the measured ratio
is $c_2/c_1 = 0.83$.}
\label{tab:ols}
\begin{tabular}{lrrr}
\toprule
...
all 18 models on log-log axes. Every MoE model (red) sits above the
...
single-node deployment~\cite{lmsys}. In all seven panels for models of 200B+
total parameters, the floor is set by a vertically integrated neocloud (six
of seven by DeepInfra alone); hyperscalers list identical models at
2--5$\times$ the floor; and participation collapses with footprint, from 17
providers for a 117GB model to three for a 1TB model. A corollary result is
...
To our knowledge this paper provides the first hedonic estimate of the
sparsity elasticity of price, formalizing an observation made informally on
five models by Scher~\cite{scher} and raised descriptively by Li et
al.~\cite{li}, and the first market-structure evidence that large-MoE serving
is gated by scale. For information-systems and economics audiences, the study
...
\bibitem{rosen} Rosen, S. Hedonic prices and implicit markets. \emph{Journal
